    ...
    static int bfs() {
        ...
        while(!que.isEmpty()) {
            ...
            for(int i = 0; i < size; i++) {
                ...
                boolean stand = false; // 제자리에 서있어야하는지
                for(int k = 0; k < 4; k++) {
                    int nx = x + dx[k];
                    int ny = y + dy[k];

                    if(nx>=1&& ny>=1&& nx<=N&&ny<=M && check[nx][ny][z] == 0) {
                        if(count%2 == 0 && arr[nx][ny] == 1) stand = true; // 밤이면서 부술벽이있을때

                        if (count%2 == 1 && z > 0 &&arr[nx][ny] == 1){
                            check[nx][ny][z] = count;
                ...
                if(stand) que.offer(new Point(x,y,z)); // 제자리에 서있기
            }
        }
        return -1;
    }
